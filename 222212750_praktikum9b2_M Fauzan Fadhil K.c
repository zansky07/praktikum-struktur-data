...
void printSelectedLevel(struct node* root, int level){
    if(root == NULL)
        return;
    if(level == 1)
        printf("%s ", root->data);
    else if(level > 1){
        printSelectedLevel(root->left, level-1);
        printSelectedLevel(root->right, level-1);
    }
}

void printCustom(struct node* root){
    for(int i = 1; i <= getHeight(root); i++){
       
        printSelectedLevel(root, i);
        printf("\n");
    }
}
//search
